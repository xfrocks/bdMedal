	...
	protected function _rebuildMedal($lastAwarded = null) {
		...
		if (empty($disableRebuildMedal)) {
			...
			if (empty($lastAwarded)) {
				$awardeds = $awardedModel->getAllAwarded(array('medal_id' => $this->get('medal_id')), array('order' => 'award_date', 'direction' => 'desc', 'limit' => 1));
	...
	protected function _getFields() {
		return array(
			'xf_bdmedal_awarded' => array(
				'awarded_id' => array('type' => 'uint', 'autoIncrement' => true),
				'medal_id' => array('type' => 'uint', 'required' => true),
				'user_id' => array('type' => 'uint', 'required' => true),
				'username' => array('type' => 'string', 'length' => 50, 'required' => true),
				'award_date' => array('type' => 'uint', 'default' => XenForo_Application::$time)
			)
		);
	}
